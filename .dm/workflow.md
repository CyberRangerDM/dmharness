...
所有 clarify 阶段的 AI Agent 与 Human 对话式讨论，必须使用 active grill-me 行为。Codex 中优先使用 `.codex/skills/grill-me/SKILL.md`，其他平台使用共享源 `.dm/skills/grill-me.md`。
...
| `clarifying` | 按 active grill-me 行为提问并形成需求简报 | Main Agent + Human | brief 调整确认后由 Main Agent 自动进入 `designing` |
...
- `clarifying` 阶段只做三件事：按 active grill-me 行为向 Human 提问；把澄清后的需求总结写入 `.dm/tasks/[task-id]/brief.md`；写完后询问 Human 是否还需要调整 `brief.md` 内容。
- `clarifying` 进行中的逐轮问题和回答只保留在当前对话上下文中；不要在每个问题结束后追加到 `.dm/tasks/[task-id]/events.jsonl`，也不要用 `.dm/tasks/[task-id]/summary.md` 滚动记录每轮进展。等 Human 交互讨论足以形成总结后，再一次性写入 `brief.md`、刷新 `summary.md` 并记录必要事件。
- 如果 Human 表示不需要调整，或 Human 已经直接调整完 `brief.md` 并明确完成，Main Agent 读取最新 `brief.md` 并进入 `designing`。如果 Human 要求继续调整，Main Agent 留在 `clarifying`，继续按 active grill-me 行为提问或根据 Human 反馈更新 `brief.md`。
...
- 操作：Main Agent 按 active grill-me 行为与 Human 对话，围绕任务计划和需求逐项提问，直到能够总结出可供设计阶段使用的需求简报。
- 讨论 skill：Codex 中优先读取并遵循 `.codex/skills/grill-me/SKILL.md`；其他平台读取并遵循 `.dm/skills/grill-me.md`。
- Clarify 行为：
  - 每次只问一个主要问题，不把工作流状态汇报当作主要输出。
  - 正在进行的 clarify 提问应看起来像原生 grill-me 输出；除非 Human 明确询问或阻塞需要说明，否则不要在问题前输出 task id、路径、命令日志、phase 说明或文件创建状态。
  - 问题使用 Human 当前语言，并带明确的问题标题，例如 `Question 1:` 或 `第 1 个问题：`。
  - 每个问题都立即给出 Main Agent 的推荐答案；中文场景优先使用 `我的推荐答案：` 这类本地化标签。
  - 沿决策树从上游依赖到下游细节逐项解决。
  - 对产品、应用或系统类需求，默认先澄清核心用户/受众以及他们要完成的工作；只有在 Human 已经说明这一点后，才优先询问产品形态、交付面、框架、数据源或实现架构。
  - 当问题是在让 Human 选择方向时，推荐答案之后应给出紧凑的编号选项列表，且选项只服务于这一个主要问题。
  - 能通过读取代码库、项目文件或 `.dm` 产物查明的事实，由 Main Agent 自行探索，不询问 Human。
  - 进行中的中间回答不写入 `events.jsonl`，也不触发 `summary.md` 的每轮更新；Agent 应在对话内保留工作上下文，直到可以写入正式 `brief.md`。
- 写入 brief：
