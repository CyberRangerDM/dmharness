...
所有 clarify 阶段的 AI Agent 与 Human 对话式讨论，还必须遵循 `.dm/specs/grill-me-discussion.spec.md`。
...
- `dm start` 创建任务后，工作流只在 `clarifying` 阶段进行人机交互。Clarify 完成并写入 `brief.md` 后，Main Agent 必须自动完成 design、design review、working、testing、accepting、summary、done 和内部返工回流，不再要求 Human 通过 `dm continue` 推进。
...
- 每轮澄清还必须遵循 grill-me 规范：一次只问一个主要待确认点，沿决策树逐个解决依赖，为每个问题提供 Main Agent 推荐答案和推荐理由，并通过简短的“我当前理解”暴露 Agent 的意图假设；能通过读取代码库或 `.dm` 文件协议查明的问题，不得转嫁给 Human。
- `clarifying` 不只检查轮次数量。最终 `brief.md` 必须记录 Human 意图模型、误解检查、非目标和验收样例或等价成功信号，关键误解仍开放时不得进入 `designing`。
...
- 强制交互：
  ...
  - 每轮提问应包含简短的“我当前理解”，把 Agent 对本轮相关需求的解释暴露给 Human 校正，避免后续设计建立在隐性误解上。
  ...
  - Main Agent 必须在 clarify 工作集中持续维护 Human 意图模型：痛点/触发原因、期望产物、使用者、使用场景、成功标准、非目标、偏好取舍和关键边界样例。
  - 离开 `clarifying` 前，Main Agent 必须执行误解检查：列出至少 3 个可能做错方向的解释，并标记为已排除、已接受或仍开放；关键误解仍开放时不得进入设计。
  - 离开 `clarifying` 前，Main Agent 必须收集或推断至少 2 条具体验收样例、检查方式或可观察成功信号；如果任务不适合样例，必须记录替代验收信号。
...
- 完成条件：Main Agent 确认当前 clarify 工作集中至少有三条已回答的有意义且 grill-me 合规的交互式确认记录，且核心目标、范围、约束、Human 意图模型、误解检查、验收样例和关键歧义状态足以进入设计；随后一次性写入 `.dm/tasks/[task-id]/brief.md`，并只做必要的目标性检查。满足后不再等待 Human 确认，直接进入 Step 4。
