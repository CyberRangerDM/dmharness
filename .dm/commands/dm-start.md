...
`brief.md` is the final clarify artifact, not the per-round scratchpad. Tell the human they may continue the conversation; if they directly create or edit `brief.md`, Main Agent must treat it as an external edit and merge it before finalizing. After the clarify gate is satisfied, Main Agent proceeds into design automatically, treats `design_review` as an automatic validation/persistence step, and continues the remaining phases until `done`, `blocked`, or a feedback loop.
