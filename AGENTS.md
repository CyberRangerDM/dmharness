...
Only `clarifying` requires interactive discussion with the human. Clarifying only does this: use active grill-me behavior (`.codex/skills/grill-me/SKILL.md` in Codex when available, otherwise `.dm/skills/grill-me.md`) to ask exactly one main question at a time, present the question in the user's language with an explicit question heading, include the agent's recommended answer with a localized label when appropriate, inspect locally discoverable facts before asking, write the summarized requirement to `brief.md`, then ask whether `brief.md` still needs adjustment. Ongoing clarify prompts should look like native grill-me output and should not be prefixed with task id, paths, command logs, phase explanations, or file creation status unless the human asked for workflow metadata or a blocker requires it. For product/app/system requests, resolve the primary user/audience and job-to-be-done before product shape, delivery surface, framework, data source, or implementation architecture unless the human already provided them. If no adjustment is needed or the human has finished adjusting it, reread the latest `brief.md` and enter the next phase. Other phases should proceed autonomously unless blocked by missing artifacts, platform capability limits, or an explicit rework request in the current session.
